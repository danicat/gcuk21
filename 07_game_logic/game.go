...
func (g *Game) Update() error {
	...
	case StateDrawPhase:
		switch key := g.players[g.current].Input().Read(); key {
		case KeyDefaultOrGraveyard:
			...
			g.players[g.current].Hand().Put(card)
			g.state = StateDiscardPhase
		}
	...
	case StateDiscardPhase:
		switch key := g.players[g.current].Input().Read(); key {
		case KeyDefaultOrGraveyard:
			card, err := g.players[g.current].Hand().Discard()
			if err != nil {
				log.Println(err)
			...
			if card.Rules.Target == "opponent" {
				g.Play(g.players[g.current], g.players[g.NextPlayer()], card)
			} else {
				// self or any
				g.Play(g.players[g.current], g.players[g.current], card)
			}
		...
		case KeyLeft:
			if viper.GetBool("reverse_left_right") {
				g.players[g.current].Hand().Right()
			} else {
				g.players[g.current].Hand().Left()
			}
	...
	case StateTurnEnd:
		if g.players[g.current].Distance() == viper.GetInt("rules.max_distance") {
			log.Printf("player %d cleared the game!", g.current)
			g.state = StateGameOver
			return nil
		}
		g.current = g.NextPlayer()
		g.state = StateTurnStart
...
func (g *Game) Draw(screen *ebiten.Image) {
	...
	case StateTurnStart, StateDealHand, StateDrawPhase, StateDiscardPhase, StateTurnEnd:
		...
		if g.current == 0 || !viper.GetBool("hide_cpu_hand") {
			g.players[g.current].Hand().DrawImage(screen)
		}
		...
		text.Draw(screen, fmt.Sprintf("Current Phase: Player %d - %s", g.current+1, g.state), ttfRoboto, viper.GetInt("layout.info.startX"), viper.GetInt("layout.info.startY"), color.RGBA{R: 0xFF, G: 0x00, B: 0x00, A: 0xFF})
